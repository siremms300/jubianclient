...
import { verifyEmail } from '../../utils/api'; // Import the API function
...
const Verify = () => {
  ...
  // Check for code and email in URL parameters
  useEffect(() => {
    ...
    if (codeFromUrl && emailFromUrl) {
      ...
      // Auto-submit if both code and email are present in URL
      handleAutoVerification(codeFromUrl, emailFromUrl);
    }
  }, [searchParams]);
  ...
  const handleAutoVerification = async (code, email) => {
    setIsLoading(true);
    try {
      const verificationData = { code, email };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const verificationData = { code: verificationCode, email };
      const response = await verifyEmail(verificationData);
